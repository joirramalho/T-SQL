...
SELECT
    DATEADD(HOUR, @TimeZone, xed.value('@timestamp', 'datetime2(3)')) AS CreationDate, xed.query('.') AS XEvent
FROM
	(
...
WHERE DATEDIFF(HOUR, DATEADD(HOUR, @TimeZone, xed.value('@timestamp', 'datetime2(3)')), GETDATE()  ) < 6
