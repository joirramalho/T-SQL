...
BEGIN
	...
	IF ( SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS c WHERE DATA_TYPE NOT IN ( 'text', 'image' ) AND TABLE_NAME = 'TbFaseNotaAvaliacaoPadrao' AND LEFT(COLUMN_NAME,1) <> '_' ) <> 13
	  BEGIN
	...
	IF ( @TipoOperacao = 'D' )
		INSERT	TbFaseNotaAvaliacaoPadrao_Auditoria (	Audit_DataHora,	Audit_Operacao,		Audit_IdUsuario,	[IdFaseNotaAvaliacaoPadrao], [IdBlocoAvaliacao], [StAtivo], [NomeAvaliacao], [Conteudo], [ValorMaximo], [Peso], [DataAvaliacao], [StSubstituirMenorNota], [RegraCalculoAusenciaDeNota], [StEntregaOnline], [EntregaOnlineDataLimite], [ExtensaoAceita] )
			SELECT					GETDATE(),		@TipoOperacao,		@IdUsuario,	[IdFaseNotaAvaliacaoPadrao], [IdBlocoAvaliacao], [StAtivo], [NomeAvaliacao], [Conteudo], [ValorMaximo], [Peso], [DataAvaliacao], [StSubstituirMenorNota], [RegraCalculoAusenciaDeNota], [StEntregaOnline], [EntregaOnlineDataLimite], [ExtensaoAceita]
			FROM	deleted
	ELSE
		INSERT	TbFaseNotaAvaliacaoPadrao_Auditoria (	Audit_DataHora,	Audit_Operacao,		Audit_IdUsuario,	[IdFaseNotaAvaliacaoPadrao], [IdBlocoAvaliacao], [StAtivo], [NomeAvaliacao], [Conteudo], [ValorMaximo], [Peso], [DataAvaliacao], [StSubstituirMenorNota], [RegraCalculoAusenciaDeNota], [StEntregaOnline], [EntregaOnlineDataLimite], [ExtensaoAceita] )
			SELECT					GETDATE(),		@TipoOperacao,		@IdUsuario,	[IdFaseNotaAvaliacaoPadrao], [IdBlocoAvaliacao], [StAtivo], [NomeAvaliacao], [Conteudo], [ValorMaximo], [Peso], [DataAvaliacao], [StSubstituirMenorNota], [RegraCalculoAusenciaDeNota], [StEntregaOnline], [EntregaOnlineDataLimite], [ExtensaoAceita]
