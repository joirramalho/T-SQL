...
--DECLARE @rolename sysname = 'Acesso_ViewAnyDatabase'
...
SELECT  C.name AS Ds_Login, B.name AS Ds_Usuario
INTO    #Users_Logins
...
DECLARE @Ds_Usuario_Recuperado VARCHAR(MAX), @Ds_Login_Recuperado VARCHAR(MAX)

SELECT @Ds_Login_Recuperado = Ds_Login, @Ds_Usuario_Recuperado = Ds_Usuario
FROM   #Users_Logins
...
SELECT  C.name AS Ds_Usuario, B.name AS Ds_Database_Role
INTO    #Database_Roles
...
WHERE   (C.name IN (@Ds_Usuario_Recuperado, @Ds_Login_Recuperado) OR @Ds_Usuario IS NULL)


-- Database Permissions
IF (OBJECT_ID('tempdb..#Database_Permissions') IS NOT NULL) DROP TABLE #Database_Permissions

SELECT A.class_desc AS Ds_Tipo_Permissao, A.permission_name AS Ds_Permissao, A.state_desc AS Ds_Operacao, B.name AS Ds_Usuario_Permissao, C.name AS Ds_Login_Permissao, D.name AS Ds_Objeto
INTO   #Database_Permissions
...
SELECT B.name AS Ds_Usuario, C.name AS Ds_Server_Role
INTO   #Server_Roles
FROM   sys.server_role_members	A	WITH(NOLOCK)
...
WHERE   (B.name IN (@Ds_Usuario_Recuperado, @Ds_Login_Recuperado) OR @Ds_Usuario IS NULL)
    

-- Server permissions
IF (OBJECT_ID('tempdb..#Server_Permissions') IS NOT NULL) DROP TABLE #Server_Permissions

SELECT  A.class_desc AS Ds_Tipo_Permissao, A.state_desc AS Ds_Tipo_Operacao, A.permission_name AS Ds_Permissao, C.name AS Ds_Login, B.type_desc AS Ds_Tipo_Login
