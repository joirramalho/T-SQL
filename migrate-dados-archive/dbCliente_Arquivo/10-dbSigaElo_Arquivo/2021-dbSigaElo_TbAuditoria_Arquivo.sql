-- SELECT  COUNT(*) FROM dbSigaElo_Arquivo.dbo.TbAuditoria;

SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaElo.dbo.TbAuditoria GROUP BY YEAR(DataHORA);

    -- CREATE DATABASE dbSigaElo_Arquivo

    -- DROP TABLE dbSigaElo_Arquivo.dbo.TbAuditoria

-- SELECT  TOP 100 * FROM dbSigaElo_Arquivo02.dbo.TbAuditoria


USE dbSigaElo_Arquivo;

-- CRIA TABELA DESTINO
	--SELECT * INTO dbSigaElo_Arquivo.dbo.TbAuditoria
	--    FROM dbSigaElo.dbo.TbAuditoria WHERE YEAR(DAtaHora) = 2015
...
SET IDENTITY_INSERT dbSigaElo_Arquivo.dbo.TbAuditoria ON;

INSERT INTO dbSigaElo_Arquivo.dbo.TbAuditoria ( Tabela, IdUsuario, Operacao, DataHora, ValorChave1, ValorChave2, ValorChave3, ValorChave4, ValorChave5, DescricaoOperacao, IdAuditoria )
        SELECT Tabela, IdUsuario, Operacao, DataHora, ValorChave1, ValorChave2, ValorChave3, ValorChave4, ValorChave5, DescricaoOperacao, IdAuditoria
            FROM dbSigaElo.dbo.TbAuditoria WHERE YEAR(DAtaHora) <= 2021

            
        

SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaElo.dbo.TbAuditoria 			GROUP BY YEAR(DataHORA) ORDER BY YEAR(DataHORA);
SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaElo_Arquivo.dbo.TbAuditoria 	GROUP BY YEAR(DataHORA) ORDER BY YEAR(DataHORA);
...
BEGIN
	...
	FROM dbSigaElo.dbo.TbAuditoria
