-- SELECT  COUNT(*) FROM dbSigaCoesi_Arquivo.dbo.TbAuditoria;

SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaCoesi.dbo.TbAuditoria GROUP BY YEAR(DataHORA);

    -- CREATE DATABASE dbSigaCoesi_Arquivo

    -- DROP TABLE dbSigaCoesi_Arquivo.dbo.TbAuditoria

-- SELECT  TOP 100 * FROM dbSigaCoesi_Arquivo02.dbo.TbAuditoria


USE dbSigaCoesi_Arquivo;

-- CRIA TABELA DESTINO
	--SELECT * INTO dbSigaCoesi_Arquivo.dbo.TbAuditoria
	--    FROM dbSigaCoesi.dbo.TbAuditoria WHERE YEAR(DAtaHora) = 2015
...
SET IDENTITY_INSERT dbSigaCoesi_Arquivo.dbo.TbAuditoria ON;

INSERT INTO dbSigaCoesi_Arquivo.dbo.TbAuditoria ( Tabela, IdUsuario, Operacao, DataHora, ValorChave1, ValorChave2, ValorChave3, ValorChave4, ValorChave5, DescricaoOperacao, IdAuditoria )
        SELECT Tabela, IdUsuario, Operacao, DataHora, ValorChave1, ValorChave2, ValorChave3, ValorChave4, ValorChave5, DescricaoOperacao, IdAuditoria
            FROM dbSigaCoesi.dbo.TbAuditoria WHERE YEAR(DAtaHora) <= 2021

            
        

SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaCoesi.dbo.TbAuditoria 			GROUP BY YEAR(DataHORA) ORDER BY YEAR(DataHORA);
SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaCoesi_Arquivo.dbo.TbAuditoria 	GROUP BY YEAR(DataHORA) ORDER BY YEAR(DataHORA);
...
BEGIN
	...
	FROM dbSigaCoesi.dbo.TbAuditoria
