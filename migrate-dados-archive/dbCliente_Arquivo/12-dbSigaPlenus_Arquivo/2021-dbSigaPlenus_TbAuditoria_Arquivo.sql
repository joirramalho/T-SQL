-- SELECT  COUNT(*) FROM dbSigaPlenus_Arquivo.dbo.TbAuditoria;

SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaPlenus.dbo.TbAuditoria GROUP BY YEAR(DataHORA);

    -- CREATE DATABASE dbSigaPlenus_Arquivo

    -- DROP TABLE dbSigaPlenus_Arquivo.dbo.TbAuditoria

-- SELECT  TOP 100 * FROM dbSigaPlenus_Arquivo02.dbo.TbAuditoria


USE dbSigaPlenus_Arquivo;

-- CRIA TABELA DESTINO
	--SELECT * INTO dbSigaPlenus_Arquivo.dbo.TbAuditoria
	--    FROM dbSigaPlenus.dbo.TbAuditoria WHERE YEAR(DAtaHora) = 2015
...
SET IDENTITY_INSERT dbSigaPlenus_Arquivo.dbo.TbAuditoria ON;

INSERT INTO dbSigaPlenus_Arquivo.dbo.TbAuditoria ( Tabela, IdUsuario, Operacao, DataHora, ValorChave1, ValorChave2, ValorChave3, ValorChave4, ValorChave5, DescricaoOperacao, IdAuditoria )
        SELECT Tabela, IdUsuario, Operacao, DataHora, ValorChave1, ValorChave2, ValorChave3, ValorChave4, ValorChave5, DescricaoOperacao, IdAuditoria
            FROM dbSigaPlenus.dbo.TbAuditoria WHERE YEAR(DAtaHora) <= 2021

            
        

SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaPlenus.dbo.TbAuditoria 			GROUP BY YEAR(DataHORA) ORDER BY YEAR(DataHORA);
SELECT	YEAR(DataHORA),   COUNT(*) FROM dbSigaPlenus_Arquivo.dbo.TbAuditoria 	GROUP BY YEAR(DataHORA) ORDER BY YEAR(DataHORA);
...
BEGIN
	...
	FROM dbSigaPlenus.dbo.TbAuditoria
