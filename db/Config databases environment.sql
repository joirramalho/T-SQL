...
SELECT
	name,
	CASE
		WHEN page_verify_option_desc <> 'CHECKSUM' THEN 'ALTER DATABASE [' + name + '] SET PAGE_VERIFY CHECKSUM;'
		ELSE ''
	END AS [page verify option]
FROM
	sys.databases DB
WHERE
	CASE
...
SELECT
	name,
	CASE
		WHEN is_auto_close_on = 1 THEN 'ALTER DATABASE [' + name + '] SET AUTO_CLOSE OFF;'
		ELSE ''
	END AS [auto close]
FROM
	sys.databases DB
...
SELECT
	name,
	CASE
		WHEN is_auto_create_stats_on = 0 THEN 'ALTER DATABASE [' + name + '] SET AUTO_CREATE_STATISTICS ON;'
		ELSE ''
...
SELECT
	name,
	CASE
		WHEN is_auto_update_stats_on = 0 THEN 'ALTER DATABASE [' + name + '] SET AUTO_UPDATE_STATISTICS ON;'
		ELSE ''
