...
CREATE TABLE ##TEMP ( [Object_ID] INT, Problem VARCHAR(1024) )

 
INSERT INTO ##TEMP ( [Object_ID], [Problem] ) 
      ...
      therefore, the constraint may not hold for all rows. */
        ...
        SELECT sys.tables.object_id, 'can''t be indexed'
          ...
          select a.object_id, 'hardcoded database name reference: ' + type_desc COLLATE SQL_Latin1_General_CP1_CI_AI + ' ' + object_name(a.object_id) + ' in ' + DB_NAME() + ' pointing to ' + x.name
...
SELECT 
	( SELECT Object_Schema_Name(Object_ID) + '.' + Object_Name(Object_ID) ) AS [TableName],
	   STUFF( ( SELECT '; ' + US.[Problem] 
          FROM  ##TEMP US
          WHERE US.[Object_ID] = t1.[Object_ID]
          FOR XML PATH('') ), 1, 1, '') AS [Problem]
FROM  ##TEMP t1
GROUP BY t1.[Object_ID], t1.[Problem]
ORDER BY 1
